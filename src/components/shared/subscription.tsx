...
export default function Subscription() {
    ...
    return (
        <motion.div
            ...
            className="w-full h-136.5 flex flex-col relative px-30 pt-18.5 overflow-hidden">
            <Image fill src="/images/home/subscription.png" alt="Subscription" className="object-cover" />

            <motion.span
                variants={subscriptionTitle}
                className="font-galderglynn text-[128px] leading-30 z-2 text-white/60">
                Subscribe for our newsletter.
            </motion.span>

            <div className="flex flex-row w-full z-2 justify-between">
                <motion.span
                    variants={subscriptionSubTitle}
                    className='text-white/60 text-5 max-w-135 leading-5 mt-6'>
                    Stay updated with the latest news, offers, and exclusive content delivered straight to your inbox!
                </motion.span>

                <motion.div
                    variants={subscriptionForm}
                    className="mt-6.5 p-6 rounded-sm bg-white/12 backdrop-blur-[20px] flex flex-row items-center gap-4 z-10 relative">
                    <div className="flex flex-row items-center h-14 px-6 py-4 gap-3 max-w-95.75 overflow-hidden w-95.75 bg-white rounded-sm">
